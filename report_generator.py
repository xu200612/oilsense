...
极端期不以单点方向预测为核心，报告应说明常规模型在结构突变下的失效边界，并把P10/P50/P90解释为企业银行压力测试区间和尾部风险覆盖工具。
...
6. 限制说明：说明这是XGBoost风险区间+历史情景匹配+AI分析的辅助判断；极端期方向命中只作参考，核心看风险识别、区间覆盖和企业敞口压力测试，模型表现以页面回测为准。
...
def generate_report(
    ...
    if is_black_swan and bs_signals:
        ...
        sys_msg   = (
            ...
            "极端期不承诺单点方向预测，重点说明模型失效边界、压力区间覆盖和企业银行风控动作。"
