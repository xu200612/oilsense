...
def _build_context(signals: dict, recent_news: list = None) -> str:
    ...
    context = (
        ...
        "- 当前霍尔木兹风险：约20%海运石油贸易通道受扰；净供应缺口需扣除替代管道、库存释放、绕行与需求萎缩，不能直接等同于全球20%供应中断\n"
        + news_text +
        "━━━━━━━━━━━━━━━━━━━━━━━"
    )
    return context
...
请作为OilSense企业银行风险分析助手，模拟花旗杯企业银行风险管理场景，提供一份“地缘政治冲击—油价—企业敞口—银行动作”闭环分析。不要声称自己是花旗员工或代表花旗出具意见。
（中文，专业金融语言，总字数不超过900字）：

重要口径：极端地缘政治期间不以单点方向预测为核心。请把模型输出解释为“风险区间/压力测试/尾部覆盖”工具，重点说明系统如何识别常规模型失效边界、切换到情景分析，并给出企业银行风控动作。

**1. 事件定性与严重程度**
（结合封锁天数、油轮降幅、VIX、GDELT/新闻情绪、历史类比，评定严重程度：轻微/中等/严重/极端，并给出理由）

**2. 供需缺口与传导链**
（区分“通过量受扰”和“净供应中断”；从替代路线、战略储备、绕行、需求侧响应四个维度解释市场如何再平衡）

**3. 分情景价格路径预测（未来1-4周）​**
请用表格呈现：
| 情景 | 触发条件 | WTI价格区间/方向 | 企业现金流影响 |
|------|---------|----------------|----------------|
| 快速缓解 | ... | ... | ... |
| 僵持延续 | ... | ... | ... |
| 全面升级 | ... | ... | ... |
...
**4. 企业银行敞口**
- 航空/航运：燃油与绕行成本、现金流与套保需求
- 化工/炼化：原料成本、库存重估与价差传导
- 能源贸易商：保证金、信用证与贸易融资需求
- 能源生产商/进口商：偿债覆盖、采购节奏与汇率联动

**5. 银行动作与对冲建议**
（3条具体可执行建议，注明适用客户、工具类型/动作、触发条件、期限）

**6. 每日关键监测指标**
（3-4个指标，注明当前值、阈值和含义。若上下文没有当前值，写“当前数据不足”。）

硬性约束：只使用上下文给出的数据，不要编造最新新闻、价格、概率、API来源或模型准确率。结尾写“本报告由OilSense AI辅助生成，仅供风险管理参考，不构成投资建议。”"""
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": (
                     "你是OilSense企业银行风险分析助手，模拟企业银行大宗商品风险分析框架，"
                     "专注地缘政治风险对大宗商品价格的影响。"
                     "分析需有具体数字支撑，避免空泛，给出可执行建议。"
                     "极端期不承诺单点方向预测，重点解释压力区间、尾部风险覆盖和银行风控动作。"
                     "只使用上下文给出的数据；数据缺失时说明当前数据不足，不要编造实时事实或模型表现。"
